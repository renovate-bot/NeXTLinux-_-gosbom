...
See: [how to specify file globs](https://github.com/nextlinux/gosbom/tree/v0.70.0/syft/pkg/cataloger/javascript/cataloger.go#L16-L21)
and an implementation of the [package-lock.json parser](https://github.com/nextlinux/gosbom/tree/v0.70.0/syft/pkg/cataloger/javascript/cataloger.go#L16-L21) for a quick review.
...
Catalogers must fulfill the interface [found here](https://github.com/nextlinux/gosbom/tree/v0.70.0/syft/pkg/cataloger.go). 
This means that when building a new cataloger, the new struct must implement both method signatures of `Catalog` and `Name`.

A top level view of the functions that construct all the catalogers can be found [here](https://github.com/nextlinux/gosbom/tree/v0.70.0/syft/pkg/cataloger/cataloger.go).
When an author has finished writing a new cataloger this is the spot to plug in the new catalog constructor.

For a top level view of how the catalogers are used see [this function](https://github.com/nextlinux/gosbom/tree/v0.70.0/syft/pkg/cataloger/catalog.go#L41-L100) as a reference. It ranges over all catalogers passed as an argument and invokes the `Catalog` method:

Each cataloger has its own `Catalog` method, but this does not mean that they are all vastly different.
Take a look at the `apkdb` cataloger for alpine to see how it [constructs a generic.NewCataloger](https://github.com/nextlinux/gosbom/tree/v0.70.0/syft/pkg/cataloger/apkdb/cataloger.go).
...
See [here for the APK example](https://github.com/nextlinux/gosbom/tree/v0.70.0/syft/pkg/apk_metadata.go#L16-L41).
- The other is a `parseFunction` which informs the cataloger what to do when it has found one of the above matches files.
See this [link for an example](https://github.com/nextlinux/gosbom/tree/v0.70.0/syft/pkg/cataloger/apkdb/parse_apk_db.go#L22-L102).
...
Identified packages share a common struct so be sure that when the new cataloger is constructing a new package it is using the [`Package` struct](https://github.com/nextlinux/gosbom/tree/v0.70.0/syft/pkg/package.go#L16-L31).

Metadata Note: Identified packages are also assigned specific metadata that can be unique to their environment. 
See [this folder](https://github.com/nextlinux/gosbom/tree/v0.70.0/syft/pkg) for examples of the different metadata types.
...
- [Call for new package](https://github.com/nextlinux/gosbom/blob/v0.70.0/syft/pkg/cataloger/apkdb/parse_apk_db.go#L106)
- [APK Package Constructor](https://github.com/nextlinux/gosbom/tree/v0.70.0/syft/pkg/cataloger/apkdb/package.go#L12-L27)
